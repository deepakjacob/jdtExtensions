...
public class CreateTypeUsingWizardProposal extends ChangeCorrectionProposal {
  // implements IJavaCompletionProposal, ICompletionProposalExtension6, ICommandAccess {
  ...
  public CreateTypeUsingWizardProposal(ICompilationUnit cu, Name node, int typeKind,
      ...
      case K_CLASS:
        ...
        if (isInnerType) {
          if (containerName.length() == 0) {
            setDisplayName(Messages.format(
                CorrectionMessages.NewCUCompletionUsingWizardProposal_createinnerclass_description,
                typeLabel));
          } else {
            setDisplayName(Messages
                .format(
                    CorrectionMessages.NewCUCompletionUsingWizardProposal_createinnerclass_intype_description,
          ...
          if (containerName.length() == 0) {
            ...
            setDisplayName(Messages
                .format(
                    CorrectionMessages.NewCUCompletionUsingWizardProposal_createclass_inpackage_description,
                    new String[] {typeLabel, containerLabel}));
          }
        }
        break;
      case K_INTERFACE:
        ...
        if (isInnerType) {
          if (containerName.length() == 0) {
            setDisplayName(Messages
                .format(
                    CorrectionMessages.NewCUCompletionUsingWizardProposal_createinnerinterface_description,
                    typeLabel));
          } else {
            setDisplayName(Messages
                .format(
                    CorrectionMessages.NewCUCompletionUsingWizardProposal_createinnerinterface_intype_description,
                    new String[] {typeLabel, containerLabel}));
          }
        } else {
          if (containerName.length() == 0) {
            setDisplayName(Messages.format(
                CorrectionMessages.NewCUCompletionUsingWizardProposal_createinterface_description,
                typeLabel));
          } else {
            setDisplayName(Messages
                .format(
                    CorrectionMessages.NewCUCompletionUsingWizardProposal_createinterface_inpackage_description,
                    new String[] {typeLabel, containerLabel}));
          }
        }
        break;
    ...
    if (typeKind == K_CLASS || typeKind == K_INTERFACE) {
      ...
      if (parent.getLocationInParent() == ParameterizedType.TYPE_PROPERTY) {
        String typeArgBaseName =
            name.startsWith(String.valueOf('T')) ? String.valueOf('S') : String.valueOf('T'); // use
                                                                                              // 'S'
                                                                                              // or
                                                                                              // 'T'
  ...
  private void fillInWizardPageSuperTypes(NewTypeWizardPage page) {
    ...
    if ((fTypeKind == K_CLASS)) {
      page.setSuperClass(fNode.getFullyQualifiedName(), true);
    } else if (fTypeKind == K_INTERFACE) {
      List<String> superInterfaces = new ArrayList<String>();
      superInterfaces.add(fNode.getFullyQualifiedName());
  ...
  public Object getAdditionalProposalInfo(IProgressMonitor monitor) {
    ...
    buf.append(JavaElementLabels.getElementLabel(fTypeContainer,
    ...
    if (superclass != null) {
      if (superclass.isClass()) {
        if (fTypeKind == K_CLASS) {
          ...
          nameToHTML(BindingLabelProvider.getBindingLabel(superclass,
              BindingLabelProvider.DEFAULT_TEXTFLAGS), buf);
        }
      } else {
